...
Eigen::Matrix<double, Tdim, 1> mpm::Mesh<Tdim>::compute_mtf_velocity_single_wave(
  ...
  try {
    ...
    for (unsigned m = 1; m <= order; ++m) {
      ...
      //零频漂移修正系数
      double amend_coeff = 1 / std::pow(1 + 0.03, m);//3.19
...
Eigen::Matrix<double, Tdim, 1> mpm::Mesh<Tdim>::interpolate_particle_mtf_velo_at_point(
  ...
  try {
    ...
    // 1. 优先在可接受的候选cell中选取“质心最近”的cell，避免边界线上的随机归属
    double min_candidate_distance = std::numeric_limits<double>::max();
    for (size_t i = 0; i < all_cells.size(); ++i) {
      auto cell = all_cells[i];
      if (!cell) continue;
      VectorDim xi;
      if (cell->is_ctbpoint_in_cell(point_coord, &xi)) {
        const double distance = (cell->centroid() - point_coord).norm();
...
mpm::CTBQuantities<Tdim> mpm::Mesh<Tdim>::interpolate_at_point(
  ...
  try {
    ...
    // 定位外推点在哪个cell中
    // 方法1：使用CTB专用检查（处理边界点），并以“质心最近”确定唯一cell
    double min_candidate_distance = std::numeric_limits<double>::max();
    for (size_t i = 0; i < all_cells.size(); ++i) {
      auto cell = all_cells[i];
      if (!cell) continue;
    ...
    if (!found_cell)
      throw std::runtime_error("No valid cell found for extrapolation point");
