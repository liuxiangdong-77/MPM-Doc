...
inline bool mpm::Cell<Tdim>::is_ctbpoint_in_cell(
  ...
  // First use standard point-in-cell check (already accepts points on edges)
  if (this->is_point_in_cell(point, xi)) return true;

  // If point is too far away from this cell, reject early
  (*xi).fill(std::numeric_limits<double>::max());
  if (!this->approx_point_in_cell(point)) return false;
  ...
  // Reject invalid local coordinates
  if (local_coords.hasNaN()) return false;
