...
inline bool mpm::Cell<Tdim>::is_point_in_cell(
  ...
  // 检查变换后的坐标是否在单元格内：如果元素是三角形或四面体，则在 0 和 1 - x - y - z 平面之间；否则在 -1 和 1 之间。同时，检查变换后的坐标是否恰好位于单元格的边缘上。
...
inline bool mpm::Cell<Tdim>::is_ctbpoint_in_cell(
  ...
  // 先走标准判定：内部点直接返回
  if (this->is_point_in_cell(point, xi)) return true;

  // 远离当前单元的点直接拒绝
  if (!this->approx_point_in_cell(point)) return false;

  // CTB边界点容差与“塞入内部”微调量
  const double boundary_tolerance = 1.E-8;
  const double ctb_tolerance = 1.E-10;
  ...
  // 仅接受位于（或非常接近）当前单元边界/内部的点
  for (unsigned i = 0; i < Tdim; ++i) {
    if (std::isnan(local_coords(i))) return false;
    if (local_coords(i) < -1.0 - boundary_tolerance ||
        local_coords(i) > 1.0 + boundary_tolerance)
      return false;
  }

  // 对落在边界线上的点，微调到单元内部，避免后续xi越界
  *xi = local_coords;
  for (unsigned i = 0; i < Tdim; ++i) {
    if ((*xi)(i) <= -1.0) (*xi)(i) = -1.0 + ctb_tolerance;
    if ((*xi)(i) >= 1.0) (*xi)(i) = 1.0 - ctb_tolerance;
  }
