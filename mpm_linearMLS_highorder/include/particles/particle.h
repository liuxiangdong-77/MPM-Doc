...
/*=====================================================CTBlxd==============================================================*/
  ...
  bool has_ctb_history() const override;

  //! Get history variable at specific time step for CTB
  //! \param[in] var_name Base variable name (without time suffix)
  ...
  double get_history_variable(const std::string& var_name, 
  ...
  //! \param[in] time_step Time step offset (0 = current, 1 = previous, etc.)
  //! \param[in] phase Material phase  
  //! \retval velocity Velocity vector at specified time step
  VectorDim get_history_velocity(unsigned time_step = 0,
                                unsigned phase = mpm::ParticlePhase::Solid) const override;

  // //! Get history stress for CTB
  // //! \param[in] time_step Time step offset (0 = current, 1 = previous, etc.)
  // //! \param[in] phase Material phase
  // //! \retval stress Stress vector at specified time step  
  // Eigen::Matrix<double, 6, 1> get_history_stress(unsigned time_step = 0,
  //                                               unsigned phase = mpm::ParticlePhase::Solid) const override;

  // //! Get history density for CTB
  // //! \param[in] time_step Time step offset (0 = current, 1 = previous, etc.)
  // //! \param[in] phase Material phase
  // //! \retval density Density at specified time step
  // double get_history_density(unsigned time_step = 0,
  //                           unsigned phase = mpm::ParticlePhase::Solid) const override;

  //! Advance CTB history variables (should be called at end of time step)
  void advance_ctb_history(unsigned long long time_step) override;
...
/*=========================================================CTB==============================================================*/
  ...
  bool assign_mtf_acceleration(const VectorDim& mtf_acceleration) override;

  //! Return acceleration of the particle
  //! \ingroup Implicit
  VectorDim acceleration() const override { return acceleration_; }

  VectorDim mtf_acceleration() const override { return mtf_acceleration_; }
 ...
 protected:
  ...
  Eigen::Matrix<double, Tdim, 1> mtf_acceleration_; // For mtf acceleration P2G (to calculation point)

  /**
   * \defgroup ImplicitVariables Variables dealing with implicit MPM 隐式MPM时候用到的变量
   */
  /**@{*/
  //! Acceleration
