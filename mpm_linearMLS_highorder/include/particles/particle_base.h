...
class ParticleBase {
 public:
  ...
  virtual VectorDim mtf_acceleration() const = 0;
...
/*======================CTB历史变量相关lxd=======================*/
  ...
  virtual bool has_ctb_history() const = 0;

  //! Get history variable at specific time step for CTB
  virtual double get_history_variable(const std::string& var_name, 
                                     unsigned time_step = 0,
                                     unsigned phase = mpm::ParticlePhase::Solid) const = 0;

  //! Get history velocity for CTB
  virtual VectorDim get_history_velocity(unsigned time_step = 0,
                                        unsigned phase = mpm::ParticlePhase::Solid) const = 0;

  // //! Get history stress for CTB
  // virtual Eigen::Matrix<double, 6, 1> get_history_stress(unsigned time_step = 0,
  //                                                       unsigned phase = mpm::ParticlePhase::Solid) const = 0;

  // //! Get history density for CTB
  // virtual double get_history_density(unsigned time_step = 0,
  //                                   unsigned phase = mpm::ParticlePhase::Solid) const = 0;

  //! Advance CTB history variables
  virtual void advance_ctb_history(unsigned long long time_step) = 0;
...
/*======================CTB历史变量相关=======================*/
  ...
  virtual bool assign_mtf_acceleration(const VectorDim& mtf_acceleration) = 0;
 ...
 protected:
  ...
  //! Material
  std::vector<std::shared_ptr<Material<Tdim>>> material_;//存储基类指针，实际可以指向任何派生类
  //! Unsigned material id
  std::vector<unsigned> material_id_;
  //! Material state history variables
  //! 物质点历史变量存储器（容器）
  std::vector<mpm::dense_map> state_variables_;//lxd2025.12.19
