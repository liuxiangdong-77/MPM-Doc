...
bool mpm::MPMExplicit<Tdim>::solve() {
  ...
  // Phase
  const unsigned phase = 0;//定义粒子相（固体）
  ...
  // Initialise mesh
  this->initialise_mesh();//这里出现了问题2025.12.16
  ...
  // Create nodal properties//lxd
  if (interface_ or absorbing_boundary_ or ctb_boundary_) mesh_->create_nodal_properties();
  ...
  // Main loop
  // 主函数循环，在这里加store_particle_history_for_ctb
  for (; step_ < nsteps_; ++step_) {
    ...
    // 施加CTB结点属性（波速）
    if(ctb_boundary_){
      mpm_scheme_->ctb_boundary_properties();// P2G加速度
      this->nodal_ctb_constraints(phase, dt_); 
    }//2025.12.15lxd
    ...
    // 存储历史变量
    mpm_scheme_->store_particle_history_for_ctb(step_);//lxd
