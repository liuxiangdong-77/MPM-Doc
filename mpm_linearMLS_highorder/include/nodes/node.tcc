...
void mpm::Node<Tdim, Tdof, Tnphases>::initialise() noexcept {
  ...
  mtf_acceleration_.setZero();
  mtf_velocity_status_ = mtf_velocity_;
  mtf_velocity_.setZero();
}
...
bool mpm::Node<Tdim, Tdof, Tnphases>::assign_pressure_constraint(
  ...
  try {
    ...
    if (phase < Tnphases) {
      ...
      throw std::runtime_error("Pressure constraint phase is invalid");
...
bool mpm::Node<Tdim, Tdof, Tnphases>::compute_acceleration_velocity(
  ...
  bool status = false;
  const double tolerance = 1.0e-10;//3.19
  if (mass_(phase) > tolerance) {
    ...
    // this->mtf_acceleration_.col(phase) = 
    //     (this->external_force_.col(phase)) / this->mass_(phase);

    VectorDim mtf_velocity_update = mtf_velocity(phase);
    ...
    // if(this->mtf_velocity_.col(phase).norm() < tolerance){
    //   this->velocity_.col(phase).noalias() += this->acceleration_.col(phase) * dt;
    // }else{
    //   // this->acceleration_.col(phase) = (mtf_velocity_.col(phase) - velocity_.col(phase)) / dt;
    //   // this->velocity_.col(phase).noalias() = this->mtf_velocity_.col(phase) + this->mtf_acceleration_.col(phase) * dt;
    //   // this->velocity_.col(phase).noalias() = this->mtf_velocity_.col(phase);
    //   // this->velocity_.col(phase).noalias() = mtf_velocity_update * 10000.0;//有用的，说明加上了
    //   this->velocity_.col(phase).noalias() = mtf_velocity_update;
    // }//MTF重构速度/加速度

    if(mtf_velocity_update.norm() < tolerance){
      this->velocity_.col(phase).noalias() += this->acceleration_.col(phase) * dt;
    }else{
      this->acceleration_.col(phase) = (mtf_velocity_update - this->velocity_.col(phase)) / dt;
      this->velocity_.col(phase).noalias() = mtf_velocity_update;
    }
...
bool mpm::Node<Tdim, Tdof, Tnphases>::compute_acceleration_velocity_cundall(
  ...
  if (mass_(phase) > tolerance) {
    ...
    // double mtf_min = 1e-9;
    // if(this->mtf_acceleration_.norm() < mtf_min){
    //   this->velocity_.col(phase).noalias() += this->acceleration_.col(phase) * dt;
    // }else{
    //   this->velocity_.col(phase).noalias() += this->mtf_acceleration_.col(phase) * dt;
    // }
    ...
    if(mtf_velocity_update.norm() < tolerance){
      ...
      this->acceleration_.col(phase) = (mtf_velocity_update - this->velocity_.col(phase)) / dt;
      // this->velocity_.col(phase).noalias() = (VectorDim::Ones() - damping_factor * this->velocity_.col(phase).cwiseSign()).cwiseProduct(mtf_velocity_update);
      this->velocity_.col(phase).noalias() = mtf_velocity_update;
    }
...
// !粘性-弹簧组合型吸收边界（Viscous-Spring Absorbing boundary），粘弹性人工边界
template <unsigned Tdim, unsigned Tdof, unsigned Tnphases>
...
// !弹簧吸收边界（Spring Absorbing boundary），for MTF
template <unsigned Tdim, unsigned Tdof, unsigned Tnphases>
bool mpm::Node<Tdim, Tdof, Tnphases>::apply_spring_constraint(
    unsigned dir, double delta, double h_min, mpm::Position position) {
  bool status = true;
  try {
    ...
    if (material_ids_.size() != 0) {
      ...
      // Calculate Aborbing Traction
      Eigen::Matrix<double, Tdim, 1> spring_traction_ = material_displacement.cwiseProduct(spring_constant);
      ...
      if (Tdim == 2) {
        ...
        switch (position) {
          case mpm::Position::Corner:
            absorbing_force_ = 0.5 * h_min * spring_traction_;
            break;
          case mpm::Position::Edge:
            absorbing_force_ = h_min * spring_traction_;
            break;
          default:
            throw std::runtime_error("Invalid absorbing boundary position");
            break;
        ...
        switch (position) {
          case mpm::Position::Corner:
            absorbing_force_ = 0.25 * pow(h_min, 2) * spring_traction_;
            break;
          case mpm::Position::Edge:
            absorbing_force_ = 0.5 * pow(h_min, 2) * spring_traction_;
            break;
          case mpm::Position::Face:
            absorbing_force_ = pow(h_min, 2) * spring_traction_;
            break;
          default:
            throw std::runtime_error("Invalid absorbing boundary position");
            break;
