...
class Cell {
 public:
  ...
  //! Return particles_
  std::vector<Index> particles() const { return particles_; }//返回向量的拷贝，而不是引用
  ...
  inline bool is_ctbpoint_in_cell(const Eigen::Matrix<double, Tdim, 1>& point,
                                  Eigen::Matrix<double, Tdim, 1>* xi);//1.5
