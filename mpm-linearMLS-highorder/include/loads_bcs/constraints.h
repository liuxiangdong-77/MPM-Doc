...
#include "logger.h"
#include "mesh.h"//所以，constraints.h是上层接口
...
class Constraints {
 public:
  ...
  //using VectorDim = Eigen::Matrix<double, Tdim, 1>;//lxd
...
/*==============================================CTB================================================*/
  ...
  bool assign_nodal_ctb_constraint(
      int nset_id,
  ...
  //! \param[in] nset_id Node set IDs
  //! \param[in] ctb_constraint Constraint at node
  void assign_ctb_id_ptr(unsigned nset_id,
                         std::shared_ptr<mpm::CTBConstraint>& ctb_constraint);

  //! Get CTB constraints
  std::vector<std::shared_ptr<mpm::CTBConstraint>> ctb_ptrs() const {
    return ctb_constraint_;
  }

  //! CTB constraint node set IDs
  std::vector<unsigned> ctb_ids() const { return ctb_nset_id_; }
...
/*=================================================lxd=============================================*/
 ...
 private:
  ...
  //! List of ctb constraint ptrs
  std::vector<std::shared_ptr<mpm::CTBConstraint>> ctb_constraint_; //lxd
  //! List of ctb constraint node set Ids
  std::vector<unsigned> ctb_nset_id_;
};
}  // namespace mpm

#include "constraints.tcc"
#include "ctb_constraint.tcc"//lxd
