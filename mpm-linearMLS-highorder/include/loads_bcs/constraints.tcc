...
//! Assign nodal velocity constraints //通过结点集施加
template <unsigned Tdim>
...
//! Assign velocity constraints to nodes //通过结点逐个施加
template <unsigned Tdim>
...
/*============================================================================================================*/
//! Apply absorbing constraints to nodes  //通过结点集批量应用吸收边界条件
template <unsigned Tdim>
bool mpm::Constraints<Tdim>::assign_nodal_absorbing_constraint(
    int nset_id,//结点集ID，通过结点集批量应用吸收边界条件
    const std::shared_ptr<mpm::AbsorbingConstraint>& absorbing_constraint) {//absorbing_constraint：智能指针，指向一个AbsorbingConstraint类的实例，该类封装了吸收边界的所有参数和逻辑
  bool status = true;
  try {
    int set_id = nset_id;
    auto nset = mesh_->nodes(set_id);
    ...
    unsigned dir = absorbing_constraint->dir();//压缩波（P波）传播的方向
    double delta = absorbing_constraint->delta();//虚拟厚度，控制弹簧刚度的（spring boundary）Lysmer-Kuhlemeyer吸收边界
    double h_min = absorbing_constraint->h_min();//实际的网格尺寸
    double a = absorbing_constraint->a();//optional 无量纲 dashpot factor，对应P波
    double b = absorbing_constraint->b();//optional 无量纲 dashpot factor，对应S波
...
//! Assign absorbing constraints to nodes   //通过结点ID列表逐个应用吸收边界条件
template <unsigned Tdim>
bool mpm::Constraints<Tdim>::assign_nodal_absorbing_constraints(
  ...
  try {
    for (const auto& absorbing_constraint : absorbing_constraints) {
      // Node id
      mpm::Index nid = std::get<0>(absorbing_constraint);//结点ID，通过结点ID列表逐个应用吸收边界条件
      // Direction
...
void mpm::Constraints<Tdim>::assign_absorbing_id_ptr(
  ...
  this->absorbing_constraint_.emplace_back(absorbing_constraint);//存储约束对象指针
  this->absorbing_nset_id_.emplace_back(nset_id);//存储对应的结点集ID
}
/*============================================================================================================*/