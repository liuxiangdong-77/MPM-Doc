...
// Assign CTB Boundary Properties
template <unsigned Tdim>
inline void mpm::MPMScheme<Tdim>::ctb_boundary_properties() {
  // Initialise nodal properties
  mesh_->initialise_nodal_properties();
...
inline void mpm::MPMScheme<Tdim>::compute_particle_kinematics(
  ...
  if (damping_type == "Cundall")
    ...
    mesh_->iterate_over_nodes_predicate(
        ...
        std::bind(&mpm::NodeBase<Tdim>::status, std::placeholders::_1));//MTF重构加速度、速度增量在这里,Lysmer计算应力->加速度也在这

  // Iterate over each particle to compute updated position
  mesh_->iterate_over_particles(
      std::bind(&mpm::ParticleBase<Tdim>::compute_updated_position,
                std::placeholders::_1, dt_, velocity_update, blending_ratio));//将MTF重构的加速度：mtf_accel插值回物质点
