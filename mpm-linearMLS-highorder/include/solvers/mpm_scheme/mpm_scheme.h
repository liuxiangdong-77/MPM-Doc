...
enum class Damping { None, Cundall, Rayleigh };//lxd
...
class MPMScheme {
 public:
  ...
  MPMScheme(const std::shared_ptr<mpm::Mesh<Tdim>>& mesh, double dt);//lxd
...
/*=================================CTB======================================*/
  ...
  //! 配置CTB结点材料属性
  virtual inline void ctb_boundary_properties();//lxd
  virtual bool ctb_boundary() { return ctb_boundary_; }//lxd
...
virtual inline void store_particle_history_for_ctb(unsigned long long time_step);//lxd
...
/*=============================lxd====================================*/
 ...
 protected:
  ...
  bool ctb_boundary_ = true;

};  // MPMScheme class
}  // namespace mpm

#include "mpm_scheme.tcc"
#include "mpm_scheme_ctb.tcc"//lxd
