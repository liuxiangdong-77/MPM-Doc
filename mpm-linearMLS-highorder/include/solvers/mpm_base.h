...
class MPMBase : public MPM {
 public:
  ...
  //! Solve
  bool solve() override { return true; }//重写基类MPM中的solve函数，该函数执行后固定返回true值
  ...
  //！在每个时间步施加ctb约束条件 lxd
  void nodal_ctb_constraints(unsigned phase, double dt);//12.01 lxd
 ...
 private:
  ...
  // 初始化ctb边界条件(Json输入文件接口)
  void nodal_ctb_constraints(
      const Json& mesh_props, const std::shared_ptr<mpm::IOMesh<Tdim>>& mesh_io,
      unsigned phase, double dt);//lxd11.10

  //json文件字符串转为Edge_Position枚举类型
  mpm::Edge_Position string_to_edge_position(const std::string& position_str);//lxd12.1
 ...
 protected:
  ...
  bool absorbing_boundary_{false};//false: 初始化bool变量值为false

  bool ctb_boundary_{false};//lxd
