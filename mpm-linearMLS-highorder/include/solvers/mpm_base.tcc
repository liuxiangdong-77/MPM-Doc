...
mpm::MPMBase<Tdim>::MPMBase(const std::shared_ptr<IO>& io) : mpm::MPM(io) {
  ...
  tsl::robin_map<std::string, VariableType> variables = {
      ...
      {"mtf_accelerations", VariableType::Vector},//新增粒子mtf加速度
      // Tensor variables
...
void mpm::MPMBase<Tdim>::initialise_mesh() {
  ...
  // Read and assign CTB constraints
  this->nodal_ctb_constraints(mesh_props, mesh_io, phase_, dt_);//lxd //在这里给ctb_boundary_赋值为true了
...
void mpm::MPMBase<Tdim>::initialise_materials() {
  ...
  for (const auto material_props : materials) {
    ...
    const std::string material_type =
        material_props["type"].template get<std::string>();//测试成功

    // Get material id
    auto material_id = material_props["id"].template get<unsigned>();//测试成功
...
void mpm::MPMBase<Tdim>::nodal_absorbing_constraints(
  ...
  try {
    ...
    if (mesh_props.find("boundary_conditions") != mesh_props.end() &&
      ...
      // range-based for 循环，const auto& constraints是循环变量，
      // 代表容器中的每个元素，mesh_props是要遍历的容器
        // Set id
        int nset_id = constraints.at("nset_id").template get<int>();
        // Direction
        unsigned dir = constraints.at("dir").template get<unsigned>();
...
/*=====================================ctb=========================================*/
// Assign nodal ctb constraints //lxd —— 读取吸收边界条件mpm.json文件，创建吸收边界条件对象，并将其分配给相应的节点集
template <unsigned Tdim>
void mpm::MPMBase<Tdim>::nodal_ctb_constraints(
    const Json& mesh_props, const std::shared_ptr<mpm::IOMesh<Tdim>>& mesh_io,
    unsigned phase, double dt) {
  try {
    // Read and assign ctb constraints
    if (mesh_props.find("boundary_conditions") != mesh_props.end() &&
        mesh_props["boundary_conditions"].find("ctb_constraints") !=
            mesh_props["boundary_conditions"].end()) {

      console_->info("Reading ctb constraints...");

      ctb_boundary_ = true;

      // Iterate over ctb constraints
      for (const auto& constraints :
           mesh_props["boundary_conditions"]["ctb_constraints"]) {
      //range-based for 循环，const auto& constraints是循环变量，代表容器中的每个元素，mesh_props是要遍历的容器
        ...
        // Assign node set IDs and list of constraints
        constraints_->assign_ctb_id_ptr(nset_id, ctb_constraint);//将结点集和约束绑定

      }
    } else
      throw std::runtime_error("CTB constraints JSON data not found");

  } catch (std::exception& exception) {
    console_->warn("#{}: CTB conditions are undefined; {}", __LINE__,
                   exception.what());
    ctb_boundary_ = false;
  }
}

// Apply nodal ctb constraints
template <unsigned Tdim>
void mpm::MPMBase<Tdim>::nodal_ctb_constraints(unsigned phase, double dt) {
  try{

    console_->debug("Applying CTB constraints, phase = {}, dt = {}", phase, dt);//成功

    std::vector<std::shared_ptr<mpm::CTBConstraint>> ctb_constraint =
      constraints_->ctb_ptrs();// 成功
    std::vector<unsigned> ctb_nset_id = constraints_->ctb_ids();//成功
    for (unsigned i = 0; i < ctb_nset_id.size(); i++) {
      auto nset_id = ctb_nset_id.at(i);
      const auto& ctb_constraint_ptr = ctb_constraint.at(i);//成功
      bool ctb_constraints =
          constraints_->assign_nodal_ctb_constraint(nset_id, ctb_constraint_ptr, phase, dt);
      if (!ctb_constraints)
        throw std::runtime_error(
            "mpm::base::nodal_ctb_constraints(): Nodal ctb constraints are not properly applied");
    }
  }catch(const std::exception& exception){
    console_->error("#{}: Failed to apply CTB constraints; {}", __LINE__, exception.what());
    throw;
  }
}

// 辅助函数：将字符串转换为 Edge_Position
template <unsigned Tdim>
mpm::Edge_Position mpm::MPMBase<Tdim>::string_to_edge_position(const std::string& position_str) {
  // 转换为小写以便不区分大小写比较
  std::string lower_str = position_str;
  std::transform(lower_str.begin(), lower_str.end(), lower_str.begin(), ::tolower);
  
  if (lower_str == "none") return mpm::Edge_Position::None;
  
  // 边界面
  if (lower_str == "left") return mpm::Edge_Position::Left;
  if (lower_str == "right") return mpm::Edge_Position::Right;
  if (lower_str == "bottom") return mpm::Edge_Position::Bottom;
  if (lower_str == "top") return mpm::Edge_Position::Top;
  if (lower_str == "front") return mpm::Edge_Position::Front;
  if (lower_str == "back") return mpm::Edge_Position::Back;
  
  // 2D角点
  if (lower_str == "bottomleft") return mpm::Edge_Position::BottomLeft;
  if (lower_str == "topleft") return mpm::Edge_Position::TopLeft;
  if (lower_str == "bottomright") return mpm::Edge_Position::BottomRight;
  if (lower_str == "topright") return mpm::Edge_Position::TopRight;
  
  // 3D角点
  if (lower_str == "bottomleftfront") return mpm::Edge_Position::BottomLeftFront;
  if (lower_str == "bottomleftback") return mpm::Edge_Position::BottomLeftBack;
  if (lower_str == "topleftfront") return mpm::Edge_Position::TopLeftFront;
  if (lower_str == "topleftback") return mpm::Edge_Position::TopLeftBack;
  if (lower_str == "bottomrightfront") return mpm::Edge_Position::BottomRightFront;
  if (lower_str == "bottomrightback") return mpm::Edge_Position::BottomRightBack;
  if (lower_str == "toprightfront") return mpm::Edge_Position::TopRightFront;
  if (lower_str == "toprightback") return mpm::Edge_Position::TopRightBack;
  
  // 支持旧的位置命名（如果必要）
  if (lower_str == "corner") {
    console_->warn("Using deprecated 'corner' position, please use specific corner position like 'bottomleft'");
    return mpm::Edge_Position::BottomLeft; // 默认返回一个角点
  }
  if (lower_str == "edge") {
    console_->warn("Using deprecated 'edge' position, please use specific edge position like 'left'");
    return mpm::Edge_Position::Left; // 默认返回一个边
  }
  if (lower_str == "face") {
    console_->warn("Using deprecated 'face' position, please use specific face position like 'front'");
    return mpm::Edge_Position::Front; // 默认返回一个面
  }
  
  // 如果找不到匹配的位置，返回None并警告
  console_->warn("Unknown Edge_Position string '{}', using None", position_str);
  return mpm::Edge_Position::None;
}
/*=====================================ctb=========================================*/
