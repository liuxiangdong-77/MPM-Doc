...
/*=======================================CTB======================================================*/
  ...
  VectorDim compute_instantaneous_wave_speed(unsigned phase) const override;

  void update_mtf_velocity(VectorDim& new_velocity, unsigned phase) override;//lxd

  void update_displacement_mtf(VectorDim& new_displacement, unsigned phase) override;

  VectorDim mtf_acceleration(unsigned phase) const override {return mtf_acceleration_.col(phase);}

  VectorDim mtf_velocity(unsigned phase) const override {return mtf_velocity_.col(phase);}

  void update_mtf_acceleration(VectorDim& mtf_accel, unsigned phase) override;
...
/*=======================================lxd=====================================================*/
 ...
 private:
  ...
  //! mtf_Acceleration
  Eigen::Matrix<double, Tdim, Tnphases> mtf_acceleration_;//用来存MTF得到的加速度
  //! mtf_Velocity
  Eigen::Matrix<double, Tdim, Tnphases> mtf_velocity_;
  //! mtf_velocity_status
  Eigen::Matrix<double, Tdim, Tnphases> mtf_velocity_status_;//上一步的速度
