...
/*====================================================CTB================================================*/
  ...
  virtual VectorDim compute_boundary_normal(mpm::Edge_Position boundary_position) const = 0;

  virtual VectorDim compute_instantaneous_wave_speed(unsigned phase) const = 0;//纯虚函数语法标记，不是赋值操作

  virtual void update_mtf_velocity(VectorDim& new_velocity, unsigned phase) = 0;

  virtual void update_displacement_mtf(VectorDim& new_displacement, unsigned phase) = 0;

  virtual VectorDim mtf_acceleration(unsigned phase) const = 0;

  virtual VectorDim mtf_velocity(unsigned phase) const = 0;

  virtual void update_mtf_acceleration(VectorDim& mtf_accel, unsigned phase) = 0;
