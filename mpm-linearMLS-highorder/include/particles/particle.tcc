...
std::shared_ptr<void> mpm::Particle<Tdim>::pod() const {
  ...
  if (this->material() != nullptr) {
    ...
    if (state_variables_[mpm::ParticlePhase::Solid].size() > 100)
      throw std::runtime_error("# of state variables cannot be more than 100");
    unsigned i = 0;
    auto state_variables = (this->material())->state_variables();
    for (const auto& state_var : state_variables) {
...
void mpm::Particle<Tdim>::initialise() {
  ...
  this->vector_properties_["mtf_accelerations"] = [&]() { return mtf_acceleration(); };//MTF
...
bool mpm::Particle<Tdim>::assign_material_state_vars(
  ...
  if (material != nullptr && this->material(phase) != nullptr &&
    ...
    auto mat_state_vars = (this->material(phase))->initialise_state_variables();//在linear_elastic_ctb.h里已经注册了
  ...
  try {
    ...
    if (material != nullptr) {
      ...
      state_variables_.at(phase) =
          material_[phase]->initialise_state_variables();//创建带时间后缀的历史变量名 eg：“density_t0”
      status = true;
    } else {
...
void mpm::Particle<Tdim>::compute_updated_position_flip(
  ...
  for (unsigned i = 0; i < nodes_.size(); ++i) {
    ...
    // if(nodes_[i]->mtf_acceleration(mpm::ParticlePhase::Solid).norm() < tolerance ){
    //   nodal_acceleration.noalias() +=
    //       shapefn_[i] * nodes_[i]->acceleration(mpm::ParticlePhase::Solid);
    // }else{
    //   nodal_acceleration.noalias() +=
    //       shapefn_[i] * nodes_[i]->mtf_acceleration(mpm::ParticlePhase::Solid);
    // }//将边界结点mtf_accel插值回物质点

    nodal_acceleration.noalias() +=
        shapefn_[i] * nodes_[i]->acceleration(mpm::ParticlePhase::Solid);

    // nodal_mtf_acceleration.noalias() +=
    //     shapefn_[i] * nodes_[i]->mtf_acceleration(mpm::ParticlePhase::Solid);
  }

  //Update acceleration(initialized from zero)
  //this->mtf_acceleration_.noalias() = nodal_acceleration;//每步覆写

  // Update particle velocity from interpolated nodal acceleration
  this->velocity_.noalias() += nodal_acceleration * dt;


  // this->velocity_.noalias() += nodal_acceleration * dt + nodal_mtf_acceleration * dt;
  ...
  this->coordinates_.noalias() += nodal_velocity * dt; //在这里改了
