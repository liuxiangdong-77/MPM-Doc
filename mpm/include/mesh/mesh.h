...
class Mesh {
 public:
  ...
  // 新增非常量版本（返回引用）
  Vector<NodeBase<Tdim>>& nodes_refer(unsigned set_id) {
    return (set_id == -1) ? this->nodes_ : node_sets_.at(set_id);
  }//lxd
  ...
  //! \retval cell Cell pointer
  std::shared_ptr<mpm::Cell<Tdim>> cell(mpm::Index cell_id) {
    // 检查是否存在，避免未定义行为
    if (map_cells_.find(cell_id) != map_cells_.end()) {
  ...
  //! \retval particle Particle pointer
  std::shared_ptr<mpm::ParticleBase<Tdim>> particle(mpm::Index particle_id) {
    if (map_particles_.find(particle_id) != map_particles_.end()) {
      return map_particles_[particle_id];
    }
    return nullptr;
  }//lxd
  ...
  //! \param[in] particle_id Particle id
  //! \retval exists True if particle exists
  bool has_particle(mpm::Index particle_id) const {
    return map_particles_.find(particle_id) != map_particles_.end();
  }//lxd
...
/*====================================CTB===================================*/
  ...
  bool create_nodal_ctb_constraint(int set_id,
                                   const std::shared_ptr<mpm::CTBConstraint>& constraint, unsigned phase, double dt);

  void store_particle_history_for_ctb(unsigned phase);

  void apply_ctb_constraints(unsigned phase, double dt);

  //! Apply CTB constraint to a single node
  ...
  std::vector<VectorDim> split_corner_boundary_normals(Edge_Position boundary_position);

  VectorDim interpolate_particle_mtf_velo_at_point(
        const VectorDim& point_coord,
        unsigned phase,
        int time_offset);

  VectorDim mls_interpolate_velo_at_point(
        const VectorDim& point_coord,  // 外推点坐标（插值目标点）
        const std::vector<VectorDim>& valid_particle_coords,  // 搜索域内有效粒子坐标
        const std::vector<VectorDim>& valid_particle_accels,  // 搜索域内有效粒子加速度（来自mtf_acceleration()）
        const std::vector<double>& valid_particle_weights);

  VectorDim weighted_average_velo_fallback(
  ...
  //! Interpolate physical quantities at given point
  mpm::CTBQuantities<Tdim> interpolate_at_point(
      const VectorDim& point_coord,
      unsigned phase,
      int time_offset);

  //! MLS interpolation
  mpm::CTBQuantities<Tdim> mls_interpolate_at_point(
      const VectorDim& point_coord,
  ...
  mpm::CTBQuantities<Tdim> weighted_average_fallback(
  ...
  //! Compute Riemann invariant R1
  double compute_riemann_invariant_R1(
      unsigned phase, const mpm::CTBQuantities<Tdim>& quantities,
      const VectorDim& boundary_normal, double wave_speed);

  //! Compute binomial coefficient
  double binomial_coefficient(unsigned N, unsigned m);

  //! Compute MLS weight function
  double compute_mls_weight(double distance, double support_radius);

  //! Compute MLS basis functions
  void compute_mls_basis(const VectorDim& point_coord, 
                         const VectorDim& particle_coord,
                         Eigen::VectorXd& basis_values);

  //! Get particle quantities (current or historical)
  void get_particle_quantities(
      const std::shared_ptr<mpm::ParticleBase<Tdim>>& particle,
...
/*==========================================================================*/
 ...
 private:
  ...
  //! 结点CTB约束
  std::vector<std::shared_ptr<mpm::CTBConstraint>> ctb_constraints_;//lxd
  ...
  //! CTB Logger
  std::shared_ptr<spdlog::logger> ctb_console_;//lxd
